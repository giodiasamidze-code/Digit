...
export async function deleteInternalChat(chatId) {
  const firestore = requireDb()
  const chatRef = doc(firestore, 'internalChats', chatId)
  ...
  await deleteCollectionDocuments(firestore, ['internalChats', chatId, 'messages'])

  await Promise.all(
    participantIds.map((ownerId) =>
      deleteDoc(doc(firestore, 'users', ownerId, 'internalChatInbox', chatId)),
    ),
  )
