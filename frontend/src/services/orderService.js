...
import { findOrCreateOpenConversation } from './chatService'
...
export function partitionCustomerOrders(orders) {
  const active = orders.filter((order) =>
    [ORDER_STATUS.NEW, ORDER_STATUS.ASSIGNED, ORDER_STATUS.IN_PROGRESS].includes(order.status),
  )
  const archived = orders.filter((order) =>
