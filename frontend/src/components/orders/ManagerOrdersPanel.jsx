...
function ManagerOrdersPanel({ managerName, initialOrderId, onError }) {
  ...
  const selectedOrderIdResolved = useMemo(() => {
    if (selectedOrderId && visibleOrders.some((order) => order.id === selectedOrderId)) {
      return selectedOrderId
    }
    if (initialOrderId && visibleOrders.some((order) => order.id === initialOrderId)) {
      return initialOrderId
    }
    return visibleOrders[0]?.id ?? null
  }, [selectedOrderId, visibleOrders, initialOrderId])
          ...
          {loading ? (
            ...
            visibleOrders.map((order) => (
              <button
                key={order.id}
                type="button"
                className={`orders-item ${selectedOrderIdResolved === order.id ? 'orders-item--active' : ''}`}
